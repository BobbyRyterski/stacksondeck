...
#
require_relative 'lib/stacksondeck/metadata'

include StacksOnDeck

`which gtar` # Necessary on OS X
TAR = $?.exitstatus.zero? ? 'gtar' : 'tar'

desc 'Package StacksOnDeck for Docker, Linux and OS X'
...
# desc 'Package StacksOnDeck into a Docker container'
task docker: %w[ package:linux ] do
  sh 'docker build -t stacksondeck .'
  latest_image = "docker images | grep stacksondeck | head -n 1 | awk '{ print $3 }'"
  sh "docker tag -f `#{latest_image}` sczizzo/stacksondeck:#{VERSION}"
  sh "docker tag -f `#{latest_image}` sczizzo/stacksondeck:latest"
  # sh "docker push sczizzo/stacksondeck"
  clean
end
...
def bundle_install
  ...
  sh 'mkdir pkg/tmp'
  sh 'cp -R stacksondeck.gemspec Readme.md LICENSE VERSION Gemfile Gemfile.lock bin lib pkg/tmp'
...
def create_package target
  bundle_install
  package_name = "stacksondeck-#{VERSION}-#{target}"
  package_file = ::File.join Dir.pwd, 'pkg', "#{package_name}.tar.gz"
  package_dir = ::File.join Dir.pwd, 'pkg', package_name
  output = ::File.join Dir.pwd, 'pkg', "stacksondeck_#{VERSION}_amd64.deb"
  sh "rm -rf #{package_dir}"
  sh "rm -rf #{output}" if target =~ /linux/
  sh "mkdir -p #{package_dir}/sod"
  sh "cp -R bin #{package_dir}/sod"
  sh "mkdir #{package_dir}/sod/ruby"
  sh "#{TAR} -xzf pkg/traveling-ruby-#{TRAVELING_RUBY_VERSION}-#{target}.tar.gz -C #{package_dir}/sod/ruby"
  sh "cp pkg/sod.sh #{package_dir}"
  sh "cp -pR pkg/vendor #{package_dir}/sod/vendor"
  sh "cp -R stacksondeck.gemspec Readme.md LICENSE VERSION Gemfile Gemfile.lock lib #{package_dir}/sod/vendor"
  sh "mkdir #{package_dir}/sod/vendor/.bundle"
  sh "cp pkg/bundler-config #{package_dir}/sod/vendor/.bundle/config"
  if !ENV['NO_EXT']
    sh "#{TAR} -xzf pkg/thin-#{THIN_VERSION}.tar.gz -C #{package_dir}/sod/vendor/ruby"
    sh "#{TAR} -xzf pkg/eventmachine-#{EM_VERSION}.tar.gz -C #{package_dir}/sod/vendor/ruby"
  end
  if !ENV['NO_FPM'] && target =~ /linux/
    sh %Q~
      fpm --verbose \
        -s dir -t deb -C #{package_dir} \
        -n stacksondeck -v #{VERSION} \
        --license "#{LICENSE}" \
        --description "#{SUMMARY}" \
...
def clean
  ...
  sh 'rm -f pkg/stacksondeck-*-linux-*.tar.gz'
end